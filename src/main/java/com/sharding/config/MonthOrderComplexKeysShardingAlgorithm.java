...
@Slf4j
@Component
public class MonthOrderComplexKeysShardingAlgorithm implements ComplexKeysShardingAlgorithm<String> {
    ...
    public Collection<String> doSharding(Collection<String> databaseNames, ComplexKeysShardingValue<String> complexKeysShardingValue) {
        ...
        if (CollectionUtils.isEmpty(dates)) {
            ...
            for (String ruleNo : ruleNos) {
                Collection<String> strings = baseComplexKeysShardingAlgorithm.rangeOfTables(databaseNames, dateRange, ruleNo, AGENT_ORDER_SUFFIX);
                list.addAll(strings);
            }
            return list;
        } else {
